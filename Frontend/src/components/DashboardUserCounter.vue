<template>
    ...
    <RouterLink :class="['text-decoration-none', switchTheme.color]" to="/Events"> 
        <v-icon>mdi-chevron-down</v-icon>TOTAL REGISTERED STUDENTS
    </RouterLink>
    </v-toolbar-title>
  ...
  export default {
    ...
    async fetchEnrolledStudentsCount() {
      try {
        const response = await axios.get("http://127.0.0.1:8000/api/registered-students-count", {
          headers: {
            Authorization: `Bearer ${this.$store.getters.getToken}`,
          },
        });
        this.totalEnrolledStudents = response.data.count;
      } catch (error) {
